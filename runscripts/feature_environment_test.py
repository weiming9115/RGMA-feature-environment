        ...
        if len(self.locate_env_data) == 0:
            ...
            for t in range(idx_length):
                ...
                for var in [var_name]:
                    ...
                    # processing when latitude does not exceed 60 degree because of GPM-IMERG boundary
                    if (lat_min >= -60) and (lat_max <=60):
                        ...
                        data_var_merged = xr.merge(data_var) # merge variables into one xr.dataset, t-loop
                        data_chunk.append(data_var_merged)
                        time_chunk.append(time_sel)
            
                    else:
                        print('track_id: {}...{}'.format(track_id,timestamp))  
                        print('latitude exceeding available data range...')
                        data_var_merged = xr.merge(data_var) # merge variables into one xr.dataset, t-loop
                        # filled with nan
                        data_var_merged[var] = data_var_merged*np.nan
                        data_chunk.append(data_var_merged)
                        time_chunk.append(time_sel)
