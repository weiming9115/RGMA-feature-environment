            ...
            for t in range(idx_length):
                ...
                for var in [var_name]:
                    ...
                    with xr.open_dataset(filename) as data_file:
                        ...
                        elif (lon_min < data_file.lon[0]) and (lon_max >= data_file.lon[-1]):
...
#                        data_extract_re = data_extract.interp(lon=np.linspace(data_extract.lon.min(), data_extract.lon.max(),int(lon_range/dlon)+1),
#                                          lat=np.linspace(data_extract.lat.min(), data_extract.lat.max(),int(lat_range/dlat)+1))
                        data_extract_re = data_extract.interp(lon=np.linspace(data_extract.lon.min(), data_extract.lon.max(),int(lon_range/dlon)+1),
                                          lat=np.linspace(data_extract.lat.min(), data_extract.lat.max(),int(lat_range/dlat)+1))

                        # converting lat-lon into x-y coordinates
                        data_extract_xy = data_extract_re.assign_coords(x=("lon", np.arange(len(data_extract_re.lon)))
                                                                      , y=("lat", np.arange(len(data_extract_re.lat))))
