        ...
        if len(self.locate_env_data) == 0:
            ...
            for t in range(idx_length):
                ...
                for var in [var_name]:
                    ...
                    data_full_path = data_dir / data_str
                    var_infile = self.variable_infile[var]
                    # modify the default file string with datetime info
                    # e.g, /global/cfs/cdirs/m4374/input_datasets/year2_EC-Earth3P-HR_highressst-present_r3i1f1p1/CMIP6HighResMIPEC-Earth-ConsortiumEC-Earth3P-HRhighresSST-presentr3i1p1f1grv20190509/pr/pr_3hr_EC-Earth3P-HR_highresSST-present_r3i1p1f1_gr_YYYY01010000-YYYY12312100.nc
                    tmp = str(data_full_path).replace('X',var).replace('YYYY',year).replace('MM',month)
                    ...
                    if filename.exists():
                        ...
                        with xr.open_dataset(filename) as data_file:
                            ...
                            data_file = data_file.sel(time=time_sel)[var_infile]
                            data_file = coordinates_processors(data_file)
                            # standardize lat/lon coordinates for slicing 
                            data_file.coords['lon'] = self.lon_env
                            data_file.coords['lat'] = self.lat_env

                            # nearest lat/lon grid point of the MCS centroid
                            lat_cen_nearest = self.lat_env[np.argmin(abs(self.lat_env - lat_cen[t]))]
                            lon_cen_nearest = self.lon_env[np.argmin(abs(self.lon_env - lon_cen[t]))]             

                            lat_min = lat_cen_nearest - lat_range/2
                            lat_max = lat_cen_nearest + lat_range/2
                            lon_min = lon_cen_nearest - lon_range/2
                            lon_max = lon_cen_nearest + lon_range/2
                            ...
                            if (lon_min >= 0) and (lon_max <= 360):
                                ...
                                idx_lon = np.argmin(np.abs(data_file.lon.values - lon_cen_nearest))
                                idx_lat = np.argmin(np.abs(data_file.lat.values - lat_cen_nearest))
                                ...
                                # update lat/lon 
                                lon_array = np.arange(lon_cen_nearest - lon_range/2, lon_cen_nearest + lon_range/2 + dlon_env, dlon_env)
                                lon_array = np.where(lon_array <= 360, lon_array, lon_array-360)
                                lat_array = np.arange(lat_cen_nearest - lat_range/2, lat_cen_nearest + lat_range/2 + dlat_env, dlat_env)
                        ...
                        raise ValueError("The input environment data does not exist: {}".format(filename))
