    ...
    for track in data_T.tracks.values:
        ...
        for t in data_T.time.values:
            ...
            try:
                ...
                Buoy_CAPE = np.full(T2m.shape,np.nan)
                Buoy_SUBSAT = np.full(T2m.shape,np.nan)
                Buoy_TOT = np.full(T2m.shape,np.nan)
                thetae_bl_array = np.full(T2m.shape,np.nan)
                thetae_lt_array = np.full(T2m.shape,np.nan)
                thetae_sat_lt_array = np.full(T2m.shape,np.nan)
